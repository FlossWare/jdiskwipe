...
diskwipe-java is a disk wiping utility that securely overwrites free disk space with zero-filled files. It uses multiple worker threads to efficiently fill the target directory until the disk is full, making previously deleted data unrecoverable.
...
This creates `target/diskwipe-java-1.0.jar` with all dependencies.
...
```bash
java -jar target/diskwipe-java-1.0.jar <directory>
```
...
```bash
java -jar target/diskwipe-java-1.0.jar /tmp/secure-wipe
```

**Use 8 threads with 20MB buffer:**
```bash
java -jar target/diskwipe-java-1.0.jar -t 8 -b 20971520 /tmp/secure-wipe
```

**Wipe multiple directories:**
```bash
java -jar target/diskwipe-java-1.0.jar /tmp/wipe1 /tmp/wipe2 /tmp/wipe3
```

**Skip confirmation (for scripts):**
```bash
java -jar target/diskwipe-java-1.0.jar -y /tmp/secure-wipe
```
...
View build status: https://github.com/FlossWare/diskwipe-java/actions
